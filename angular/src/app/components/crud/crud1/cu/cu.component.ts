...
export class CuComponent implements OnInit {
  ...
  // datos: any  = [];
  public datos: Array<any>  = new Array<any> ();
  ncs = '' ;
  constructor(
  ...
  getValue( col: string): string {
    const tmp = this.datos[col];
    if (tmp === undefined) {return ''; }
    return tmp ;
  }
  getValueKey( col: string) {
    const tmp = this.datos[col];
    if (tmp === undefined) {return ''; }
    return this.datos[col];
  }
  ngOnInit() {
    this.crudModel = this.crudEService.getCrud(this.data.data.url);
    if (this.data.data.id !== 'new') {
      const url = this.crudModel.url + `/${this.data.data.id}`;
