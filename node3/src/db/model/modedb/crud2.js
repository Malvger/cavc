...
module.exports = {
    getCrudModel: async(req, res) => {
        try {
            let id = req.params.id;

            const pool = await getConnection();
            let sql = "select * from sy_tb_crud2_model where name ='" + id + "';";
            //console.log(sql);
    ...
    getStructure: async(req, res) => {
        try {
            ...
            const pool = await getConnection();
            let sql = "select *  from sy_tb_crud2 where crud='" + id + "';";
            //console.log(sql);
            const result = await pool.request().query(sql);
            //console.log(result.recordset);
            res.json(result.recordset);
    ...
    getCrudElement: async(req, res) => {
        try {
            let tl = req.params.tl;

            const pool = await getConnection();
            let sql = "select * from " + tl + ";";
            //console.log(sql);
            const result = await pool.request().query(sql);
            //console.log(result.recordset);
            res.json(result.recordset);
